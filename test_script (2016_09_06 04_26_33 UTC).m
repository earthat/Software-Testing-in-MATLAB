...
for fn = fieldnames(TcmbErrs)'
    TcmbErrsClean.(fn{:}) = TcmbErrs.(fn{:})( ~all(TcmbErrs.(fn{:})==Inf, 2) & ~all(TcmbErrs.(fn{:})==0, 2), : );
end
...
rpos = 1;
cpos = 1;
for fn = fieldnames(TcmbErrsClean)'
    [r c] = size(TcmbErrsClean.(fn{:}));
    TcmbResults(rpos:rpos+r-1, cpos:cpos+c-1) = TcmbErrsClean.(fn{:});
    rpos = rpos + r;
    cpos = cpos + c;
end
...
for n = 1:length(TcmbX)
    TcmbY(n) = 100*sum(TcmbMaxErrs>=TcmbX(n))/length(TcmbMaxErrs);
end
